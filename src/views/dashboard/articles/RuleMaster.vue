    ...
    <div class="card" style="margin-top: 5px;">
      <div class="card-body">
                  ...
                  <div class="col-md-10">
                    <div class="tag-input">
                      <div class="tag" style="margin: 1px 1px 1px;" v-for="(tag, tagIndex) in formElement.includesTags"
                        :key="tagIndex">
                        {{ tag.data }}
                        <a @click="changeState(tag, 'includes', tagIndex)" :class="tag.isSensitive === false ? 'btn btn-primary' : 'btn btn-secondary'">
                          <svg 
        ...
        <div style="float: right;">
          <button style="margin-top:4px" @click="submitForm" class="btn btn-success">Save Rule</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    const shouldClearLocalStorage = this.$route.query.clear === true; // Check if 'clear' query parameter is set to 'true'
  ...
  methods: {
    ...
    submitForm() {
      ...
      if (rule_data.rule_name !== '') {
        // all-rules
        axios.post('/collections/get-user-rules/', rule_data, { withCredentials: true })
          .then(response => {
            const notification = {
      ...
      // window.localStorage.setItem('formElements', JSON.stringify(this.formElements));
      // window.localStorage.setItem('formElements', JSON.stringify(rule_data));

    },
    loadFormFromLocalStorage() {
      // Load form data from local storage
      const savedFormElements = window.localStorage.getItem('formElements');
      // console.log(JSON.parse(savedFormElements).tableData == undefined ? [] : JSON.parse(savedFormElements).tableData)
      if (savedFormElements) {
        this.formElements = JSON.parse(savedFormElements).formElements;
        this.selection = JSON.parse(savedFormElements).rule_type;
        this.update_remove_relation = JSON.parse(savedFormElements).update_remove_relation;
        this.rule_name = JSON.parse(savedFormElements).rule_name;
        this.add_criteria = JSON.parse(savedFormElements).rule_criteria_enabled;
        this.target_column = JSON.parse(savedFormElements).target_column;
        this.target_value = JSON.parse(savedFormElements).target_value;
        this.tableData = JSON.parse(savedFormElements).tableData == undefined ? [] : JSON.parse(savedFormElements).tableData;
      }
    },
    ...
    changeState(tag, tagType, index){
      tag.isSensitive = !tag.isSensitive;
      console.log(this.formElements);
